...
test_con = [15]; % 12 = w-(f+h+t), 13 = f-(w+h+t), 14 = h-(f+w+t), 15 = t-(f+h+w), 16 = n-(f+h+t) %test_con = number of the contrast used to test the voxels (in SPM.mat of the select_path)
...
elseif volume_option == 2
    ...
    if test_con == 12
        all_xyzmm = [-20 -94 -4; -30 -48 43; -40 6 30; -50 36 13; -48 -54 -20; -68 -44 6]; %% VISUAL WORDS-(h+f+t), RH synesthetes,  lOcc, lIPS, mIFG, iIFG, VWFA, STS.
    elseif test_con == 13
        all_xyzmm = [42 -56 -20; 52 -64 16; -38 -51 -20]; %% VISUAL FACES-(w+h+t), rh synesthetes, FFA, OFA, lFFA, no difference.
    elseif test_con == 14
        all_xyzmm = [22 -76 -12; -18 -78 -12; 28 -54 -7; -28 -46 -7]; %% VISUAL HOUSES-(w+f+t), rh synesthetes, post rPPA, post lPPA, ant rPPA, ant lPPA, no difference.
    elseif test_con == 15
        all_xyzmm = [-25 -54 12; 30 -48 -12; -45 -68 -2; 48 -64 -7]; %% VISUAL TOOLS-(h+f+w), rh synesthetes, lPPA, rPPA, lOTA, rOTA, no difference.
    elseif test_con == 16
        all_xyzmm = [32 -61 43; -45 -41 43]; %% VISUAL NUMBERS-(h+f+t), rh synesthetes, rIPS, lIPS, no difference.
    end
...
%                 xyz = find(tvol>=criticalt); % uncomment if you don't want the values to be sorted
    ...
    if volume_option == 1
        save(sprintf('%s/best_voxels_in_vis_spmT%04d.mat',...
