...
for j = 1 : size(S,1)
    if ~isempty(find(~cellfun(@isempty,(regexp({S_syn.name},S(j).name)))))
        mask_cov_syn(j,1) = 1;
    else
        mask_cov_syn(j,1) = 0;
    end
...
for j = 1 : size(S,1)
    if ~isempty(find(~cellfun(@isempty,(regexp({S_con.name},S(j).name)))))
        mask_cov_con(j,1) = 1;
    else
        mask_cov_con(j,1) = 0;
    end
...
vector_cov_syn1 = vector_age(mask_cov_syn==1);
vector_cov_syn2 = vector_hand(mask_cov_syn==1);
vector_cov_con1 = vector_age(mask_cov_con==1);
vector_cov_con2 = vector_hand(mask_cov_con==1);
...
for k = 1 : length(S_syn)
    subj_of_int         = path_to_scan_syn{k};
    name_subj_of_int    = S_syn(k).name;
    controls_subj       = path_to_scan_con;
    for tmp_pair = 1 : size(pairs,1)
        if regexp(name_subj_of_int, sprintf('%s', pairs{tmp_pair,1}))
            ind         = tmp_pair;
        end
    end
    
    mask                = find(~cellfun(@isempty,(regexp({S_con.name}, sprintf('%s', pairs{ind,2})))));
    controls_subj(mask) = '';    
    cov_g1         = vector_cov_syn1(k);
    cov_g2         = vector_cov_con1;
    cov_g2(mask)      = '';
    ...
    for c = 1:length(all_contrast)
        ...
        res_dir = sprintf('%s%s/%s', res_dir, '_vs_con_age_covariate', names{c});
        if ~isdir(res_dir)
            mkdir(res_dir)
        end
        dinfo = dir(res_dir);
        ...
        %% for control group:
        scans2 = {};
        for k = 1:numel(controls_subj)
            vol_name = fullfile(controls_subj{k});
            vol_name = sprintf('%s/%s,1', controls_subj{k}, contrast);
...
for k = 1 : length(S_con)
    subj_of_int         = path_to_scan_con{k};
    name_subj_of_int    = S_con(k).name;
    controls_subj       = path_to_scan_con;
    for tmp_pair = 1 : size(pairs,1)
        if regexp(name_subj_of_int, sprintf('%s', pairs{tmp_pair,2}))
            ind         = tmp_pair;
        end
    end
