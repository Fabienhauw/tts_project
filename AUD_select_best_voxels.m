...
per_vox = 10; % percentage of voxels you want to keep: 1-100%;
pvalue = 0.001;
...
if volume_option == 1
    ROIfiles = {};
    ROIfiles = {
        'absolute/path/to/ROI1.nii',... %absolute path for ROI1
        'absolute/path/to/ROI2.nii',... %absolute path for ROI2, and so on.
        };
    ROInames = {'Roi1','Roi2'};
    all_xyzmm = ROInames;
...
for zz=1:length(test_con)
    ...
    if diff_ref
        save_name = sprintf('best_voxels_in_aud_spmT%04d_based_on_%s_spmT%04d_%s_peaks', current_con, ref_mod, select_con, peaks);
    else
...
for tmp_anal = 1 : size(results, 1)
    ...
    for s = 1 : size(anal,2)
        ...
        %         test = fitlm([anal(s).activation(anal(s).group_mask == 1), anal(s).activation(anal(s).group_mask == 2)], score_pca)
...
fprintf('\np-values for:               lOcc,     rOcc,  lIPS,  rIPS,    SMA,     mIFG,    iIFG,   VWFA,  lSTS,  rSTS.')
