...
res_dir_base = sprintf('/network/lustre/iss02/cohen/data/Fabien_official/SYNESTHEX/second_level/Vis/loc/ANOVA_s5_without_resting_%s_to_%s_s8', S_effect(1).name, S_effect(end).name);
...
%-----------------------------------------------------------------
cd(fullfile(D,S(1).name, 'Vis/loc/stats_s5_without_resting'));
cont = [1 : 5];
% faces, houses, tools, numbers, words
%-----------------------------------------------------------------
...
for con=1:ncon
    for s=1:totsub
        sub=subname{s};
        P{(con-1)*totsub+s} =	sprintf(['/network/lustre/iss02/cohen/data/Fabien_official/SYNESTHEX/final_images/%s/Vis/loc/stats_s5_without_resting/scon_%04d.nii'],sub,cont(con));
    end
end
...
Xvalues = {...
    faces, houses, tools, numbers, words, images, ...
