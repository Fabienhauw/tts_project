...
for tmp_con = 1 : length(all_con)
    ...
    for k = 1 : numel(S_effect)
        for tmp_ROI = 1 : numel(ROIs_names)
            ...
            matlabbatch{i}.spm.util.voi.adjust = NaN; % no need because we just need the mask to extract signal from it during RS.
            matlabbatch{i}.spm.util.voi.session = 1;
            matlabbatch{i}.spm.util.voi.name = sprintf('%s_%d_%d_%d_%dmm_sph_ROI_adapted_to_aud_con%d', ROIs_names{tmp_ROI}, ROIs_coord(tmp_ROI, :), roi_sph_rad, select_con);
...
%% visual rois
% coord from TTS network (speech > baseline, syn > con)
ROIs_names = {'SMG',      'VWFA',     'lIPS',     'MFG',    'lpSTG'};
ROIs_coord = [-48 -44 23; -50 -54 -14; -32 -48 40; -40 6 30; -62 -31 3]; 
select_con = 12;
clear matlabbatch
i=1;
for k = 1 : numel(S_effect)
