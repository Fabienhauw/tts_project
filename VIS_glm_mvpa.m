...
for k = a:b
    new_spm_1   = fullfile(D,S(k).name,'Vis/loc/mvpa_without_resting_10mm');
    ...
    time_load=fullfile(D,S(k).name,'Vis/loc/cpt_data/Timedata_');
    time_load = sprintf('%s%s%s',time_load,S(k).name,'_Vis.mat');
    load(time_load);
...
% 
    ...
    faces           = [repmat([1 0 0 0 0],1,nb_repeat)];
    houses          = [repmat([0 1 0 0 0],1,nb_repeat)];
    tools           = [repmat([0 0 1 0 0],1,nb_repeat)];
    numbers         = [repmat([0 0 0 1 0],1,nb_repeat)];
    words           = [repmat([0 0 0 0 1],1,nb_repeat)];
