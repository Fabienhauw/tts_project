...
for k=1:numel(S)
 ...
 for k = 1:nb_subj
    ...
    spm_file = dir('SPM*mat');
    if ~isempty(spm_file)
        spm_file = spm_file.name;
        delete(spm_file);
    end
    mask= dir('brain*.nii');
    mask = fullfile(path_to_scan{k},'anat',mask(1).name);
    path_to_stats1=fullfile(path_to_scan{k}, 'Aud/stats/chaperon');
    path_to_stats2=fullfile(path_to_scan{k}, 'Aud/stats/run/dcm_model_param_modul_endroit');
    ...
    matlabbatch{i}.spm.util.voi.roi{1}.spm.contrast = 2;  %this is the contrast reverse-rest in the "standard" SPM of chaperon rouge
...
%     matlabbatch{i}.spm.util.voi.roi{3}.sphere.move.global.mask = 'i2';
    ...
    matlabbatch{i}.spm.util.voi.adjust = f_con;
    matlabbatch{i}.spm.util.voi.session = 1;
    matlabbatch{i}.spm.util.voi.name = 'dcm_vwfa_sphere'; %comes from the visual localizer because the vwfa from FLC-controls is too small;
