...
nbreg = 3;
lpstg = 1; % or visual cortex
smg = 2; 
vwfa = 3; 

% basic matrices
A=eye(nbreg);
% A(vwfa,lpstg) = 0; % both extremities do not communicate;
% A(lpstg,vwfa) = 0; % both extremities do not communicate;
...
for aa=1:length(a)
    if (a{1,aa}(smg,lpstg)==0 & a{1,aa}(lpstg,smg)==0)  | (a{1,aa}(vwfa,smg)==0 & a{1,aa}(smg,vwfa)==0)
        a{1,aa}=[];
    end
end
...
for struct_model=1:length(a)
    ...
    for modul_model1 = 1 : size(result,1)
        cout_model = cout_model + 1 ;
        a_bis{cout_model,struct_model}(:,:,1) = a{1,struct_model};
        a_bis{cout_model,struct_model}(:,:,2) = reshape(result(modul_model1,:), n, m)';
    end
end
clear a
