...
S_multi = S;
S_multi(mask_multi) = [];
syn_multi_group = repmat({'syn_multi'}, 1, length(S_multi));

mask_one =  cellfun(@isempty,(regexp({S.name},'Sujet02|Sujet03|Sujet04|Sujet09|Sujet20')));

% mask_gauch =  ~cellfun(@isempty,(regexp({S.name},'Control')));
S_one = S;
S_one(mask_one) = [];
syn_one_group = repmat({'syn_one'}, 1, length(S_one));

categ = [syn_multi_group, syn_one_group, con_group];
nsub(1) = length(S_multi); % synesthetes projectors
nsub(2) = length(S_one); % synesthetes associators
nsub(3) = length(S_con_app); % controls
ngroups = 3;

S_effect = [S_multi ; S_one; S_con_app];
res_dir_base = sprintf('/network/lustre/iss02/cohen/data/Fabien_official/SYNESTHEX/second_level/Aud/loc/ANOVA_s5_multi_one_without_resting_%s_to_%s_s8', S_effect(1).name, S_effect(end).name);
